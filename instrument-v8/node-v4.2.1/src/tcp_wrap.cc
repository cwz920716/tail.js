...
void TCPWrap::Initialize(Local<Object> target,
  ...
  env->SetProtoMethod(t, "node_enter_request_handler", Node_EnterRequestHandler);
  env->SetProtoMethod(t, "node_exit_request_handler", Node_ExitRequestHandler);
  env->SetProtoMethod(t, "node_inside_of_request_handler", Node_InsideOfRequestHandler);
...
void TCPWrap::DoPrint(const FunctionCallbackInfo<Value>& args) {
  TCPWrap* wrap = Unwrap<TCPWrap>(args.Holder());
  printf("Do Print: %p\n", wrap->UVHandle());
  double stream_addr = reinterpret_cast<uint64_t> (wrap->UVHandle());
  args.GetReturnValue().Set(stream_addr);
}

void TCPWrap::NotifyUV_Request(const FunctionCallbackInfo<Value>& args) {
  TCPWrap* wrap = Unwrap<TCPWrap>(args.Holder());
...
void TCPWrap::Node_EnterRequestHandler(const FunctionCallbackInfo<Value>& args) {
  TCPWrap* wrap = Unwrap<TCPWrap>(args.Holder());
  int reqId = args[0]->Int32Value();
  uv_tcp_t* tcp = wrap->UVHandle();
...
void TCPWrap::Node_InsideOfRequestHandler(const FunctionCallbackInfo<Value>& args) {
  TCPWrap* wrap = Unwrap<TCPWrap>(args.Holder());
  args.GetReturnValue().Set(wrap->node_handler_layers);
}
