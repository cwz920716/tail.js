...
namespace instrument {
  ...
  class FunctionWrap {
    public:
      typedef void (*hook_t) (FunctionWrap *w);
      
    private:
      Persistent<Function> _handle;
      int _hash;
      ...
      static hook_t createFun, beforeCall, afterCall;
      static bool createFun_en, beforeCall_en, afterCall_en;

    public:
      FunctionWrap (Local<Function> local, int hash);
      FunctionWrap (Local<Function> local);

      ~FunctionWrap() { _handle.Reset(); }

      Persistent<Function>& persistent() { return _handle; }
      int hash() const { return _hash; }

      void setBoolFields(int index, bool value);
      bool getBoolFields(int index);
      void setPointerFields(int index, void *value);
      ...
      static bool setCreateFunHook(hook_t f) { createFun = f; createFun_en = true; return true; }
      static bool setBeforeCallHook(hook_t f) { beforeCall = f; beforeCall_en = true; return true; }
      static bool setAfterCallHook(hook_t f) { afterCall = f; afterCall_en = true; return true; }

      static void fireCreateFun(FunctionWrap *w) { 
        if (createFun_en) {
          return (*createFun)(w);
        }
        return; 
      }
      static void fireBeforeCall(FunctionWrap *w) { 
        if (beforeCall_en) {
          return (*beforeCall)(w);
        }
        return; 
      }
      static void fireAfterCall(FunctionWrap *w) { 
        if (afterCall_en) {
          return (*afterCall)(w);
        }
        return; 
      ...
      friend class FunctionHash;
      friend class FunctionEquals;
  };

  typedef std::unordered_set<FunctionWrap*>  FWSet;
  typedef std::unordered_map<int, FWSet> FunctionHashTable; //

  extern FunctionHashTable fht;
