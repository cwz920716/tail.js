...
namespace instrument {
  ...
  extern std::set<internal::JSFunction *> fset;
  
  bool enterFunction(internal::Handle<internal::JSFunction> handle) {
    ...
    internal::Handle<internal::Object> hash =
      internal::Object::GetPropertyOrElement(handle, name).ToHandleChecked();

    bool found = false;    
    if (hash->IsSmi()) {
      auto val = internal::Smi::cast(*hash);
      int hash_code = val->value();
      FWSet &wraps = fht[hash_code];
      for (auto w: wraps) {
        auto h1 = Utils::OpenPersistent(w->persistent());
        if (*h1 == *handle) {
          // handle->PrintName(stdout); internal::PrintF(" ");
          // internal::PrintF("%d\n", hash_code);
          FunctionWrap::fireBeforeCall(w);
          found = true;
          break;
        }
      }
  ...
  bool exitFunction(internal::Handle<internal::JSFunction> handle) {
    HandleScope handle_scope(Isolate::GetCurrent());
    internal::Isolate *isolate = handle->GetIsolate();
    internal::Factory* factory = isolate->factory();
    internal::Handle<String> name = factory->InternalizeUtf8String("__serial_hash__");
...
bool traceEnter(Isolate* isolate) {
    JavaScriptFrameIterator it(isolate);
    if (!it.done()) {
      if (it.frame()->is_java_script()) {
        JavaScriptFrame* frame = it.frame();
        ...
        instrument::enterFunction(handle);
        // if ( instrument::enterFunction(handle) ) { JavaScriptFrame::PrintTop(isolate, stdout, false, true); PrintF("\n"); }
      }
    }
    return true;
}
...
bool traceExit(Isolate* isolate) {
    JavaScriptFrameIterator it(isolate);
    if (!it.done()) {
      if (it.frame()->is_java_script()) {
        JavaScriptFrame* frame = it.frame();
        JSFunction* function = frame->function();
        Handle<JSFunction> handle(function);
        instrument::exitFunction(handle);
        // if ( instrument::enterFunction(handle) ) { JavaScriptFrame::PrintTop(isolate, stdout, false, true); PrintF("\n"); }
      }
    }
    return true;
