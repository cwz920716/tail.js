...
namespace instrument {
  FunctionHashTable fht;
  FunctionWrap::hook_t FunctionWrap::createFun = NULL, FunctionWrap::beforeCall = NULL, FunctionWrap::afterCall = NULL;
  bool FunctionWrap::createFun_en = false, FunctionWrap::beforeCall_en = false, FunctionWrap::afterCall_en = false;

  bool insertFHT(int k, FunctionWrap *v) {
    FWSet empty;

    if ( fht.find(k) == fht.end() ) {
      fht[k] = empty;
    }
  ...
  bool removeFHT(int k, FunctionWrap *v) {
    if ( fht.find(k) == fht.end() ) {
      return false;
    }
  ...
  int FunctionWrap::id = 0;

  FunctionWrap::FunctionWrap (Local<Function> local, int hash) {
    // HandleScope handle_scope(Isolate::GetCurrent());
    assert(persistent().IsEmpty());
    persistent().Reset(Isolate::GetCurrent(), local);
    MakeWeak();
    _hash = hash;
  }

  FunctionWrap::FunctionWrap (Local<Function> local) {
    // HandleScope handle_scope(Isolate::GetCurrent());
    assert(persistent().IsEmpty());
    persistent().Reset(Isolate::GetCurrent(), local);
    MakeWeak();
  ...
  int FunctionWrap::nextId() {
    if (id >= internal::Smi::kMaxValue - 1)
      return 0;
    return id + 1;
  }

  void FunctionWrap::setBoolFields(int index, bool value) {
    this->boolFields = value;
  }
  bool FunctionWrap::getBoolFields(int index) {
    return this->boolFields;
  }
  void FunctionWrap::setPointerFields(int index, void *value) {
    this->pointerFields = value;
  }
  void *FunctionWrap::getPointerFields(int index) {
    return this->pointerFields;
  }
  void FunctionWrap::setIntFields(int index, int value) {
    this->intFields = value;
  }
  ...
  void FunctionWrap::WeakCallback(
      const WeakCallbackData<Function, FunctionWrap>& data) {
    Isolate* isolate = data.GetIsolate();
    HandleScope scope(isolate);
    FunctionWrap* wrap = data.GetParameter();
    assert(wrap->_handle.IsNearDeath());
    assert(
        data.GetValue() == Local<Function>::New(isolate, wrap->_handle));
    removeFHT(wrap->hash(), wrap);
    wrap->_handle.Reset();
    delete wrap;
  }

  void createFunction(internal::Handle<internal::JSFunction> handle) {
    HandleScope handle_scope(Isolate::GetCurrent());
    internal::Isolate *isolate = handle->GetIsolate();

    // if (handle->shared()->IsApiFunction()) return;

    assert(!handle.is_null());     
    Local<Function> local = Utils::ToLocal(handle);
    FunctionWrap *wrap = new FunctionWrap(local);
    
    internal::Factory* factory = isolate->factory();
    internal::Handle<String> name = factory->InternalizeUtf8String("__serial_hash__");
    internal::Smi *val_smi = internal::Smi::FromInt(wrap->hash());
    internal::Handle<internal::Smi> val(val_smi, isolate);
    
    internal::JSObject::AddProperty(handle, name, val, static_cast<PropertyAttributes>(DONT_ENUM));
    
    insertFHT(wrap->hash(), wrap);
    
    FunctionWrap::fireCreateFun(wrap);
...
RUNTIME_FUNCTION(Runtime_NewClosureFromStubFailure) {
  ...
  auto funct = isolate->factory()->NewFunctionFromSharedFunctionInfo(shared, context,
                                                                pretenure_flag);
  // v8::instrument::createFunction(funct);
  return *funct;
}
